#!/usr/bin/env python3
"""Portal 同步唯一入口：先首页数据，再复盘手记。

顺序不可颠倒。首页收益曲线承载「当日账户事实」，手记正文会引用当日表现，
先刷新收益曲线再生成手记，同一次同步内才不会出现两套事实。此前两个脚本
各自独立、没有统一入口，收益曲线漏同步过一次（手记到 9/15 而收益曲线停在
9/14），这个入口就是为堵住那条缝。
...
用法::
    ...
    python3 tools/sync_portal.py --dry-run            # 两阶段都只预演
    python3 tools/sync_portal.py --source local       # 走本地 8088
    python3 tools/sync_portal.py --skip-reading       # 只刷首页数据

退出码：0 成功；1 参数错；2 首页数据失败；3 缺 ReviewNote 且未加
``--allow-missing-reading``；4 手记失败。
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Portal 同步：首页数据（收益曲线 + 市场快照）→ 复盘手记"
    )
    parser.add_argument("--date", help="目标交易日 YYYY-MM-DD，默认今天")
    parser.add_argument(
        "--dry-run", action="store_true", help="两阶段都只预演，不落盘"
    )
    parser.add_argument(
        "--source",
        choices=["cloud", "local"],
    ...
    # 第一步必须是首页数据：收益曲线是当日账户事实，手记正文会引用它。
    ...
    if args.skip_reading:
        print("跳过复盘手记（--skip-reading）")
        print("完成: " + " → ".join(done))
        return 0

    note = find_review_note(target_day)
    if note is None:
        print(f"未找到 {target_day} 的 ReviewNote，跳过复盘手记")
        print("完成: " + " → ".join(done))
        return 0 if args.allow_missing_reading else 3

    if not run_step(f"② 复盘手记（{note.name}）", [str(TOOLS / "convert_daily_note.py"), str(note)], args.dry_run):
        print("已完成: " + " → ".join(done))
        return 4
    done.append("② 复盘手记")
