...
**改动脱敏规则（`sanitize_public_review_text` / `sanitize_public_review_cell` / `anonymize_current_holdings`）时的额外要求**（2026-09-21 F12 教训）：
1. 上述测试**必须全绿后再推送**——策略变更而测试未同步，会留下几十个失败用例长期无人发现。
2. 重生成页面后**对线上产物做泄漏抽检**：拿账户数值（成本/现价/数量/仓位/止损/均线/收盘价）逐个 `count()`，必须为 0；同时确认标的名称与市场级指标（涨停家数、情绪、指数、板块涨幅）**未被误伤**。
3. **新规则先问作用域**：仅在某一分支（如 `position_row`）生效的脱敏规则最危险——同一列不同行会出现不同脱敏结果，单测只覆盖该分支时完全看不出来。
